...
// use time CSR (sstc) instead of cycle (zicntr) — emu doesn't support zicntr
static inline int64_t get_cycle_count() {
  int64_t count;
  __asm__ __volatile__ ("csrr %0, time" : "=r"(count));
  return count;
};

// Return the current value of the time counter
static inline int64_t get_time_count() {
  int64_t tc;
  __asm__ __volatile__ ("csrr %0, time" : "=r"(tc));
  return tc;
};
