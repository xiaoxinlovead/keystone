...
void fconv2d_3x3(double *o, double *i, double *f, int64_t R, int64_t C,
                 int64_t F) {
  // Column blocking: process n_ columns per iteration
  unsigned long bs;
  asm volatile("vsetvli %0, %1, e64, m2, ta, ma" : "=r"(bs) : "r"(C));

  for (uint64_t n = 0; n < C; n += bs) {
    uint64_t n_ = MIN(C - n, bs);
    double *o_blk = o + n;
    double *i_blk = i + n;
    ...
    for (int64_t r = block_size_o; r < R; r += block_size_o) {
      ...
      // Fetch n_ + F - 1 elements (padding included)
      asm volatile("vsetvli zero, %0, e64, m2, ta, ma" ::"r"(n_ + F - 1));
    f_ = f;
...
void fconv2d_vec_4xC_slice_preload_3x3(double *i, int64_t C, int64_t n_,
                                       int64_t F) {
  // Helper variables
  int64_t ldi = (C + F - 1) << 3;
...
void fconv2d_vec_4xC_slice_move_3x3(int64_t C, int64_t n_, int64_t F) {
  // Move n_+F-1 elements
  asm volatile("vsetvli zero, %0, e64, m2, ta, ma" ::"r"(n_ + F - 1));
