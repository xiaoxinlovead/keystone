...
SSH_OPTIONS="-i /home/yangxin/xs-env/keystone/build64-xs/overlay/root/.ssh/id_rsa"
...
upload_to_qemu() {
    ...
    scp ${SSH_OPTIONS} ${SCP_OPTIONS} -P 3072 $1 root@localhost:.
}

run_in_qemu() {
    echo "Running \"$1\" in QEMU ..."
    ssh ${SSH_OPTIONS} -p 3072 root@localhost "$1"
}

run_in_qemu "insmod keystone-driver.ko"

upload_to_qemu "/home/yangxin/xs-env/keystone/build64-xs/examples/tests/tests.ke"
