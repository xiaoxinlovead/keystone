...
int generic_early_init(bool cold_boot)
{
	...
	if (cold_boot) {
		if (!semihosting_enabled()) {
			...
			if (rc) {
				/* Register console directly with known UART address */
...
int generic_final_init(bool cold_boot)
{
	...
	fdt_open_into(fdt, fdt, fdt_totalsize(fdt) +
		      CONFIG_PLATFORM_NEMU_XIANGSHAN_FDT_EMPTY_SPACE * 1024);
...
struct sbi_platform platform = {
	...
	.platform_version	=
		SBI_PLATFORM_VERSION(CONFIG_PLATFORM_NEMU_XIANGSHAN_MAJOR_VER,
				     CONFIG_PLATFORM_NEMU_XIANGSHAN_MINOR_VER),
	.name			= CONFIG_PLATFORM_NEMU_XIANGSHAN_NAME,
